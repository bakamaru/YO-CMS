...
namespace YangOne.Log
{
    public class FileBaseLogger : ILogger
    {
        ...
        public FileBaseLogger(IWebHostEnvironment hostingEnvironment, ILoggerSetting loggerSetting)
        {
            ...
            Basedir = Path.Combine(hostingEnvironment.ContentRootPath, "Logs");
            TodaysDate = DateTime.Now.ToString("yyyy_MM_dd");

            if (!Directory.Exists(Basedir))
            {
                Directory.CreateDirectory(Basedir);
            }
            LogFilePath = Path.Combine(Basedir,CustomFileName+TodaysDate + ".log");
            _init();
        }
        public bool CreateFile<T>()
        {
            ...
            LogFilePath = Basedir + CustomFileName+TodaysDate + ".log";
            return true;
        }
        ...
        private void _init()
        {
            ...
            else
            {
                ...
                if (mb >= 5)
                {
                    LogFilePath = Basedir + CustomFileName+ TodaysDate + "-" + DateTime.Now.ToString("h.mm") + ".log";
                    using (var stream = File.Create(LogFilePath, 1024, FileOptions.None))
                    {
                    }
                }
