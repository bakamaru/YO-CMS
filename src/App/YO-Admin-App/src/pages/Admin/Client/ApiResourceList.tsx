                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <input
                            {...field}
                            type="text"
                            placeholder={t('ApiResource.Filter', 'Name')}
...
export default function ApiResourceList() {
    ...
    const columns = [
        ...
        {
            key: "displayName",
            label: t('ApiResource.DisplayName', 'Display Name'),
            render: (item: any) => item.displayName || t('Common.NA', 'N/A'),
        },
        {
            key: "description",
            label: t('ApiResource.Description', 'Description'),
            render: (item: any) => (
                <span className="text-sm text-gray-600 dark:text-gray-400">
                    {item.description || t('ApiResource.NoDescription', 'No description')}
                </span>
            ),
        },
        {
            key: "actions",
            label: t('ApiResource.Action', 'Action'),
            render: (row: any) => (
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (confirm(t('ApiResource.DeleteConfirm', 'Are you sure you want to delete this API resource?'))) {
                                handleDelete(row.id);
                            }
                        }}
    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            toaster.success(t('ApiResource.DeleteSuccess', 'API Resource deleted successfully!'));
            refetch();
        } catch (error) {
            console.error(error);
            toaster.error(t('ApiResource.DeleteError', 'An error occurred while deleting the API resource.'));
        }
    };
    ...
    return (
        <div className="space-y-6">
            <ComponentCard title={t('ApiResource.Title', 'API Resource Management')}>
                <>
