...
import { useDeleteApiScopeMutation, useGetApiScopesQuery } from "../../../redux/setting/clientAPI";

interface IFilter {
    Name: any;
}

const FilterApiScope = ({
    control,
    handleSubmit,
    onFilterSubmit,
    handleFilterReset,
                ...
                <Controller
                    ...
                    render={({ field }) => (
                        <input
                            {...field}
                            type="text"
                            placeholder={t('ApiScope.Filter', 'Name')}
...
export default function ApiScopeList() {
    ...
    const { data, isLoading, refetch } = useGetApiScopesQuery({ offset: offset, limit: limit, search: searchQuery });
    const [deleteApiScope] = useDeleteApiScopeMutation();

    const columns = [
        {
            key: "name",
            label: t('ApiScope.Name', 'Name'),
            render: (item: any) => (
                <Link
                    to={`/admin/openiddict/apiscope/edit?id=${item.id}`}
        ...
        {
            key: "displayName",
            label: t('ApiScope.DisplayName', 'Display Name'),
            render: (item: any) => item.displayName || t('Common.NA', 'N/A'),
        },
        {
            key: "description",
            label: t('ApiScope.Description', 'Description'),
            render: (item: any) => (
                <span className="text-sm text-gray-600 dark:text-gray-400">
                    {item.description || t('ApiScope.NoDescription', 'No description')}
                </span>
            ),
        },
        {
            key: "actions",
            label: t('ApiScope.Action', 'Action'),
            render: (row: any) => (
                <div className="flex items-center gap-2">
                    <button
                        title={t('Common.Edit', 'Edit')}
                        onClick={() => {
                            navigate(`/admin/openiddict/apiscope/edit?id=${row.id}`);
                        }}
                    ...
                    <button
                        ...
                        onClick={() => {
                            if (confirm(t('ApiScope.DeleteConfirm', 'Are you sure you want to delete this API scope?'))) {
                                handleDelete(row.id);
                            }
                        }}
    ...
    const handleDelete = async (id: string) => {
        try {
            const response = await deleteApiScope(id).unwrap();
            toaster.success(t('ApiScope.DeleteSuccess', 'API Scope deleted successfully!'));
            refetch();
        } catch (error) {
            console.error(error);
            toaster.error(t('ApiScope.DeleteError', 'An error occurred while deleting the API scope.'));
        }
    };

    useEffect(() => {
        if (data) {
            setApiScopes(data.Rows as any);
    ...
    return (
        <div className="space-y-6">
            <ComponentCard title={t('ApiScope.Title', 'API Scope Management')}>
                <>
                    <div className="flex flex-col gap-5 px-6 mb-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div className="relative">
                            <button
                                type="button"
                                onClick={() => {
                                    navigate("/admin/openiddict/apiscope/new");
                                }}
                                className="inline-flex items-center gap-2 px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600"
                            >
                                {t('ApiScope.AddNew', 'Add New')}
                            </button>
                        </div>
                    </div>
                    <DataGrid
