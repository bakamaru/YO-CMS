...
const defaultValues: FormValues = {
  ...
  IsActive: true,
  IsDefault: false,
  EmailServiceProviderSettings: [{ ProviderKey: "", ProviderValue: "" }],
};

const EmailServiceProviderForm = () => {
  ...
  const { fields, append, remove } = useFieldArray({
    control,
    name: "EmailServiceProviderSettings",
  });

  const [saveProvider, { isLoading: saving }] = useSaveEmailServiceProviderMutation();
  const [checkName] = useLazyGetEmailServiceProvidersQuery();
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const validSettings = data.EmailServiceProviderSettings.filter((s) => s.ProviderKey.trim());
      ...
      await saveProvider({
        ...
        EmailServiceProviderSettings: validSettings.length > 0 ? validSettings : undefined,
      }).unwrap();

      toaster.success(t("EmailProvider.Form.AddTitle"));
  ...
  return (
    <div className="space-y-4">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <ComponentCard title={t("EmailProvider.Form.AddTitle")}>
        ...
        <ComponentCard title={t("EmailProvider.Form.ProviderSettings")}>
              ...
              <div key={field.id} className="flex items-start gap-3">
                <div className="flex-1">
                  <input
                    ...
                    {...register(`EmailServiceProviderSettings.${index}.ProviderKey`, {
                      required: t("EmailProviderSetting.Form.Key"),
                    })}
                    className={`w-full rounded-lg border ${errors.EmailServiceProviderSettings?.[index]?.ProviderKey ? "border-red-500" : "border-gray-300"} bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:placeholder:text-gray-400`}
                  />
                  {errors.EmailServiceProviderSettings?.[index]?.ProviderKey && (
                    <p className="mt-1 text-xs text-red-500">
                      {errors.EmailServiceProviderSettings[index]?.ProviderKey?.message}
                    </p>
                  )}
                </div>
                <div className="flex-1">
                  <input
                    ...
                    {...register(`EmailServiceProviderSettings.${index}.ProviderValue`, {
                      required: t("EmailProviderSetting.Form.Value"),
                    })}
                    className={`w-full rounded-lg border ${errors.EmailServiceProviderSettings?.[index]?.ProviderValue ? "border-red-500" : "border-gray-300"} bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:placeholder:text-gray-400`}
                  />
                  {errors.EmailServiceProviderSettings?.[index]?.ProviderValue && (
                    <p className="mt-1 text-xs text-red-500">
                      {errors.EmailServiceProviderSettings[index]?.ProviderValue?.message}
                    </p>
                  )}
                </div>
                <button
        ...
        <div className="mt-3 flex justify-end gap-3">
          ...
          >
            {nameChecking ? t("Common.Loading") : t("EmailProvider.Form.AddTitle")}
          </button>
        </div>
      </form>
    </div>
...
export default EmailServiceProviderForm;
